...
export function render() {
  ...
  const insurance   = store.get('vehicleInsurance') ?? [];
  ...
  vehicles.forEach(v => {
    const sorted = trips.filter(t => t.vehicleName === v.name).sort((a, b) => a.odoReading - b.odoReading);
    const monthTrips = sorted.filter(t => String(t.date ?? '').startsWith(curYM));
    let mDist = 0, mLitres = 0;
    monthTrips.forEach(t => {
      const idx = sorted.findIndex(s => s.id === t.id);
      if (idx > 0) {
        const d = t.odoReading - sorted[idx - 1].odoReading;
        if (d > 0 && t.fuelLitres > 0) { mDist += d; mLitres += t.fuelLitres; }
      }
    });
    if (mLitres > 0 && mDist > 0) {
      const km = mDist / mLitres;
      if (bestMileage === null || km > bestMileage) { bestMileage = km; bestVehicleName = v.name; }
    }
  });
  if (el('veh-stat-best-mileage')) el('veh-stat-best-mileage').textContent = bestMileage ? bestMileage.toFixed(1) + ' km/L' : '—';
  if (el('veh-stat-best-mileage-sub')) el('veh-stat-best-mileage-sub').textContent = bestVehicleName ?? 'No trips this month';

  // 4. Documents Expiring Soon (within 30 days)
  let expiringCount = 0;
  let soonestDays = Infinity, soonestLabel = '';
  function _checkDoc(dateStr, label) {
    if (!dateStr) return;
    ...
    if (daysLeft <= 30) {
      expiringCount++;
      if (daysLeft < soonestDays) { soonestDays = daysLeft; soonestLabel = label; }
    }
  }
  docs.forEach(d => {
    _checkDoc(d.insuranceExpiry, `${d.vehicleName} Insurance`);
    _checkDoc(d.rcExpiry,        `${d.vehicleName} RC`);
  });
  insurance.forEach(ins => _checkDoc(ins.expiryDate, `${ins.vehicleName} Policy`));
  if (el('veh-stat-docs-expiring')) el('veh-stat-docs-expiring').textContent = expiringCount;
  if (el('veh-stat-docs-expiring-sub')) el('veh-stat-docs-expiring-sub').textContent =
    expiringCount === 0 ? 'All documents valid'
    : soonestDays < 0 ? `${soonestLabel} — Expired`
    : soonestDays === 0 ? `${soonestLabel} — Today!`
    : `${soonestLabel} — ${soonestDays}d left`;
  if (el('veh-stat-docs-icon')) {
    const docsIcon = el('veh-stat-docs-icon');
    docsIcon.style.background = expiringCount === 0
      ? 'linear-gradient(135deg,#6366f1,#818cf8)'
      : soonestDays < 0 ? 'linear-gradient(135deg,#ef4444,#dc2626)'
      : 'linear-gradient(135deg,#f59e0b,#d97706)';
  }

  // 5. Next Service Due
  const tripOdo = {};
  trips.forEach(l => { if (!tripOdo[l.vehicleName] || l.odoReading > tripOdo[l.vehicleName]) tripOdo[l.vehicleName] = l.odoReading; });
  const maintOdo = {};
  maintenance.forEach(r => { if (!maintOdo[r.vehicleName] || r.odoReading > maintOdo[r.vehicleName]) maintOdo[r.vehicleName] = r.odoReading; });
  const _getCurOdo = vName => manualOdo[vName] ?? (Math.max(tripOdo[vName] ?? 0, maintOdo[vName] ?? 0) || null);

  let minKmLeft = Infinity, serviceLabel = '—', serviceSub = 'No maintenance data';
  maintenance.forEach(r => {
    const curOdo = _getCurOdo(r.vehicleName);
    if (r.intervalKm > 0 && curOdo !== null) {
      const kmLeft = (r.odoReading + r.intervalKm) - curOdo;
      if (kmLeft < minKmLeft) {
        minKmLeft = kmLeft;
        serviceLabel = kmLeft <= 0 ? 'Overdue' : `${Math.round(Math.abs(kmLeft)).toLocaleString('en-IN')} km`;
        serviceSub   = `${r.type} · ${r.vehicleName}`;
      }
    }
    if (r.intervalDays > 0 && r.date) {
      ...
      const kmEquiv  = daysLeft * 10; // treat 1 day ≈ 10 km for priority comparison
      if (kmEquiv < minKmLeft) {
        minKmLeft  = kmEquiv;
        serviceLabel = daysLeft <= 0 ? 'Overdue' : `${daysLeft} days`;
        serviceSub   = `${r.type} · ${r.vehicleName}`;
      }
    }
  });
  if (el('veh-stat-next-service')) el('veh-stat-next-service').textContent = serviceLabel;
  if (el('veh-stat-next-service-sub')) el('veh-stat-next-service-sub').textContent = serviceSub;
  if (el('veh-stat-service-icon')) {
    el('veh-stat-service-icon').style.background = serviceLabel === 'Overdue'
      ? 'linear-gradient(135deg,#ef4444,#dc2626)'
      : minKmLeft < 500 || (minKmLeft < 50 && minKmLeft >= 0)
      ? 'linear-gradient(135deg,#f59e0b,#d97706)'
