...
import { computeAll, computeIncomeExpenseRatio } from './insights.js';
...
function renderHealthCard(h) {
  if (!h.hasData) {
    return `<div class="ai-section ai-health-section" style="border-color:#e2e8f040">
      <div class="ai-sec-hd" style="background:#f8fafc;border-bottom-color:#e2e8f0">
  ...
  return `<div class="ai-section ai-health-section" style="border-color:${scoreColor}40">
    <div class="ai-sec-hd" style="background:${scoreBg};border-bottom-color:${scoreColor}30">
...
function renderRecommendations(recs, projRate) {
  ...
  `<div class="ai-grid ai-grid--recs">
    ${visible.map((r, i) => {
      ...
      return `<div class="ai-rec-card${i >= AI_REC_LIMIT ? ' ai-rec-extra ai-rec-hidden' : ''}">
        <div class="ai-rec-top">
          <span class="ai-cat-label">${esc(r.category)}</span>
          <button class="ai-dismiss-btn" data-cat="${esc(r.category)}" title="Dismiss"><i class="bi bi-x-lg"></i></button>
        </div>
...
function renderRecurringExpenses(expenses) {
  ...
  (expenses ?? []).forEach(r => {
    ...
    if (!key) return;
    if (!descMap[key]) descMap[key] = { label, category: r.category, monthSet: new Set(), amounts: [] };
    descMap[key].monthSet.add(ym);
    descMap[key].amounts.push(Number(r.amount) || 0);
  });

  const recurring = Object.values(descMap)
    .filter(d => d.monthSet.size >= 2)
    .map(d => ({
      label: d.label,
      category: d.category,
      avgAmount: d.amounts.reduce((s, v) => s + v, 0) / d.amounts.length,
      monthsCount: d.monthSet.size,
    }))
...
function renderWhatIf(expenses) {
  ...
  if (entries.length === 0) return emptySection('Add expenses this month to see what-if savings.');

  const [topCat, topAmt] = entries[0];
  const save20 = Math.round(topAmt * 0.2);
  const annual = save20 * 12;

  const secondHtml = entries[1] ? (() => {
    const [cat2, amt2] = entries[1];
    const s = Math.round(amt2 * 0.2);
    return `<div class="ai-whatif-alt">
      <i class="bi bi-lightbulb me-2" style="color:#f59e0b"></i>
      Cut <strong>${esc(cat2)}</strong> by 20% → save <strong>${fmt(s)}/mo</strong> = <strong style="color:#f59e0b">${fmt(s*12)}/yr</strong>
    </div>`;
  })() : '';

  return `
  <div class="ai-whatif-wrap">
    <div class="ai-whatif-main">
      <div class="ai-whatif-icon"><i class="bi bi-stars"></i></div>
      <div class="ai-whatif-body">
        <div class="ai-whatif-label">Top opportunity</div>
        <div class="ai-whatif-headline">Cut <strong>${esc(topCat)}</strong> by 20%</div>
        <div class="ai-whatif-saving">
          <span class="ai-whatif-mo">${fmt(save20)}/mo</span>
          <span class="ai-whatif-sep">→</span>
          <span class="ai-whatif-yr">${fmt(annual)} saved per year</span>
        </div>
      </div>
    </div>
    ${secondHtml}
...
function renderDayOfWeekPattern(expenses) {
  ...
  <div class="ai-dow-wrap">
    <div class="ai-dow-insight">
      <i class="bi bi-calendar-event-fill me-1" style="color:#8b5cf6"></i>
      You spend the most on <strong>${days[peakIdx]}s</strong> — ${fmt(Math.round(totals[peakIdx]))} total across all time
    </div>
      ...
      ${totals.map((t, i) => {
        ...
        return `<div class="ai-dow-col">
          <div class="ai-dow-bar-wrap">
            <div class="ai-dow-bar" style="height:${heightPct}%;background:${isPeak ? 'linear-gradient(180deg,#8b5cf6,#a78bfa)' : '#e2e8f0'}"></div>
          </div>
          <div class="ai-dow-day${isPeak ? ' ai-dow-day--peak' : ''}">${days[i]}</div>
          <div class="ai-dow-amt${isPeak ? ' ai-dow-amt--peak' : ''}">${k}</div>
        </div>`;
      }).join('')}
    </div>
  </div>`;
...
function renderBiggestTransactions(expenses) {
  ...
  const top = [...thisMonth].sort((a, b) => (Number(b.amount)||0) - (Number(a.amount)||0)).slice(0, 5);
  ...
  return `<div class="ai-bigtxn-list">
    ${top.map((t, i) => {
      ...
      const cleanDesc = (t.description || t.category || 'Expense').replace(/\s*\[.*?\]/g, '').trim();
      return `<div class="ai-bigtxn-row">
        ...
        <div class="ai-bigtxn-info">
          <div class="ai-bigtxn-desc">${esc(cleanDesc)}</div>
          <div class="ai-bigtxn-meta">${esc(t.category ?? '')}${dateStr ? ' · ' + dateStr : ''}</div>
        </div>
        <div class="ai-bigtxn-amt" style="color:${COLORS[i]}">${fmt(Number(t.amount)||0)}</div>
      </div>`;
    }).join('')}
  </div>`;
}
...
export function render() {
  ...
  <div class="ai-page">
    ...
    <!-- Page header with refresh top-right -->
    <div class="ai-page-header">
      <div class="ai-page-header-left">
        <div class="ai-page-title"><i class="bi bi-robot me-2"></i>AI Insights</div>
        <div class="ai-page-subtitle">Predictive analysis · Anomalies · Spending intelligence</div>
      </div>
      <div class="ai-page-header-right">
        <span class="ai-last-updated"><i class="bi bi-clock"></i>${fmtTime(r.lastUpdated)}</span>
